...
import { esserver } from "@/lib/config";
import {
  get_chapter_author,
  get_books,
  get_blogs,
...
async function withEventStore<T>(fn: () => Promise<T>): Promise<T> {
  ...
  if (!connected) {
    throw new Error(
      `无法连接到 EventStore 服务器（${esserver}），请确认 eventstore 服务已启动。`
    );
  }
  return fn();
}
...
export async function fetchBrowseLogs(ids: string[]): Promise<BrowseLogs> {
  ...
  const result = await subscribeCollect<{
    code: number;
    counts?: { targetId: string; count: number }[];
...
export async function fetchHomePageData(): Promise<HomePageData> {
  try {
    const [books, blogs] = await Promise.all([fetchBooks(0, 4), fetchBlogs(0, 3)]);
